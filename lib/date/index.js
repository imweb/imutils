...
export function formatDate(fmt, date) {
  ...
  Object.keys(o).forEach((item) => {
    if (new RegExp(`(${item})`).test(fmt)) {
      if (item === 'y+') {
        fmt = fmt.replace(RegExp.$1, (`${o[item]}`).substr(4 - RegExp.$1.length));
      } else if (item === 'S+') {
        let lens = RegExp.$1.length;
        lens = lens === 1 ? 3 : lens;
        fmt = fmt.replace(RegExp.$1, (`00${o[item]}`).substr((`${o[item]}`).length - 1, lens));
      } else {
...
export function formatTime(t, fmt = 'hh:mm:ss') {
  ...
  Object.keys(o).forEach((item) => {
    if (new RegExp(`(${item})`).test(fmt)) {
      fmt = fmt.replace(RegExp.$1, RegExp.$1.length === 1 ? o[item] : (`00${o[item]}`).substr((`${o[item]}`).length));
    }
  });
  return fmt;
}
...
// const countDown = new CountDown({endTime, onStart, onUpdate, onPause, onEnd, isInit});
...
export class CountDown {
  constructor(opts) {
    ...
    this.onPause = opts.onPause; // 暂停回调
    this.onEnd = opts.onEnd; // 暂停回调
...
// 计时
// time为初始化时间
// const timer = new Timer({time, onUpdate, onStart, onPause, isInit})
export class Timer {
  constructor(opts) {
    this.time = opts.time;
    ...
    this.onPause = opts.onPause; // 计时暂停回调函数
